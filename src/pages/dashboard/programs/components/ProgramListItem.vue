...
<template>
  ...
  >
    <div>
      ...
      <p v-if="program.description" class="mt-1 text-sm text-slate-500 dark:text-slate-400">
        {{ program.description }}
      </p>
      <span
      ...
      >
        {{ program.pointsPerVisit }} {{ t('dashboard.programs.form.pointsPerVisit').toLowerCase() }}
      </span>
    </div>
    ...
    <div class="flex flex-none items-center gap-2">
      <button
        ...
        @click="emit('edit')"
      >
        {{ t('dashboard.programs.editButton') }}
      </button>
      <button
        type="button"
        class="rounded-lg border border-border px-3 py-1.5 text-xs font-semibold text-slate-700 transition-colors hover:bg-surface-sunken dark:text-slate-200"
      ...
      >
        {{
          program.isActive ? t('dashboard.programs.deactivate') : t('dashboard.programs.activate')
        }}
      </button>
    </div>
  </div>
